...
function initLocationData(){
	jQuery('.question-block, #number-scroll').each(function(){
		var holder = jQuery(this),
			btnLocation = holder.find('.btn-locate, .down-s'),
			answerBlock = '.block-right, #scrolled-block',
			activeClass = 'active',
...
function initAnchors() {
	...
	new SmoothScroll({
		anchorLinks: '.mouse-down',
		extraOffset: 0,
		activeClasses: 'parent',
		sectionActiveClass: 'content-active',
		wheelBehavior: 'none'
	});
}
...
function initAddClasses() {
	jQuery('.mouse-down').clickClass({
		classAdd: 'mouse-down-active',
		addToParent: 'task-completion-msg'
	});
}
